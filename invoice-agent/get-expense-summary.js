...
export async function getExpenseSummary(params) {
    ...
    if (params.bulan) {
        rows = rows.filter((r) => r.bulan.toLowerCase() === params.bulan.toLowerCase());
        if (rows.length === 0) {
            return {
                content: [
                    {
    ...
    const groupLabel = { kategori: "Kategori", bulan: "Bulan", vendor: "Vendor" }[params.groupBy];
