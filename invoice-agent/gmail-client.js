...
export async function getGmailClient() {
    ...
    if (!tokenPath) {
        throw new Error("gmail-token.json tidak ditemukan. Jalankan: npm run auth:gmail\n" +
            "Letakkan file di: " + TOKEN_PATHS[0]);
    }
    const oauth2 = new google.auth.OAuth2(process.env.GMAIL_CLIENT_ID, process.env.GMAIL_CLIENT_SECRET, "urn:ietf:wg:oauth:2.0:oob");
