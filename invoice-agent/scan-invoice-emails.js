...
// ── Main export ───────────────────────────────────────────────────────────────
export async function scanInvoiceEmails(params) {
    ...
    for (let i = 0; i < messages.length; i += 10) {
        ...
        const details = await Promise.allSettled(batch.map((m) => parseEmailDetail(gmail, m.id)));
    ...
    return {
        content: [
            {
                type: "text",
                text: `✅ Ditemukan ${parsed.length} email kuitansi.\n\n` +
...
async function parseEmailDetail(gmail, messageId) {
    ...
    if (!isReceiptEmail(from, subject))
        return null;
    const { bodyText, pdfText, hasPdfAttachment } = await extractContent(gmail, msg.payload, messageId);
    return {
        emailId: messageId,
        subject,
        from,
...
async function extractContent(gmail, part, messageId, depth = 0) {
    let bodyText = "";
    let pdfText = "";
...
// ── Utilities ─────────────────────────────────────────────────────────────────
function isReceiptEmail(from, subject) {
    const text = `${from} ${subject}`.toLowerCase();
    if (TRUSTED_DOMAINS.some((d) => text.includes(d)))
        return true;
